...
void device_set_render_target_internal(gs_device_t *device, gs_texture_t *tex,
				       gs_zstencil_t *zstencil, bool srgb)
{
	if (device->curSwapChain) {
		if (!tex)
	...
	if (tex && tex->type != GS_TEXTURE_2D) {
		blog(LOG_ERROR, "device_set_render_target_internal (D3D11): "
	...
	if (tex2d) {
		if (!srgb)
			rt = tex2d->renderTargetNoSrgb[0].Get();
		if (!rt) {
			rt = tex2d->renderTarget[0].Get();
			if (!rt) {
				blog(LOG_ERROR,
				     "device_set_render_target_internal (D3D11): "
...
{
	device_set_render_target_internal(device, tex, zstencil, true);
}

void device_set_render_target_no_srgb(gs_device_t *device, gs_texture_t *tex,
				      gs_zstencil_t *zstencil)
{
	device_set_render_target_internal(device, tex, zstencil, false);
}
...
device_texture_create_gdi(gs_device_t *device, uint32_t width, uint32_t height)
{
	...
	try {
		texture = new gs_texture_2d(device, width, height, GS_BGRA_SRGB,
					    1, nullptr, GS_RENDER_TARGET,
					    GS_TEXTURE_2D, true);
	} catch (const HRError &error) {
