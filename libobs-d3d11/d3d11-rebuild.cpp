...
void gs_texture_2d::RebuildSharedTextureFallback()
{
	...
	static const DXGI_FORMAT dxgi_format_view_srgb =
		ConvertGSTextureFormatViewSrgb(format);
	...
	dxgiFormatViewSrgb = dxgi_format_view_srgb;
	levels = 1;
	...
	viewDescSrgb.Format = dxgi_format_view_srgb;
...
void gs_texture_2d::Rebuild(ID3D11Device *dev)
{
	...
	if (viewDesc.Format == viewDescSrgb.Format) {
		shaderResSrgb = shaderRes;
	} else {
		hr = dev->CreateShaderResourceView(texture, &viewDescSrgb,
						   &shaderResSrgb);
		if (FAILED(hr))
			throw HRError("Failed to create resource view", hr);
	}
